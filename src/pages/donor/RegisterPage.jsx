...
export default function DonorRegisterPage() {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        setErrors({ email: [data?.message ?? 'Google Registration failed.'] });
      } finally {
        setLoading(false);
      }
    },
  ...
  const inputCls = (key) =>
    `w-full border rounded-xl py-3.5 text-sm placeholder-gray-400 bg-white focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent transition ${errors[key] ? 'border-red-400' : 'border-gray-200'}`;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-md mx-auto px-5 pt-10 pb-10 md:pt-16 md:pb-16 flex flex-col">
        ...
        {/* Back button */}
        <button onClick={() => navigate(-1)} className="self-start text-gray-700 hover:text-gray-900 mb-6 md:hidden" aria-label="Go back">
        ...
        <p className="text-gray-500 text-sm leading-relaxed mb-7">
          Get started in seconds. You can complete your profile after signing up.
        </p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-5 py-6">
          <div className="space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 animate-fade-slide-in">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} className={`${inputCls('password')} pl-11 pr-12`} placeholder="Min. 6 characters" value={form.password} onChange={set('password')} required minLength={6} />
                  <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute inset-y-0 right-3.5 flex items-center text-gray-400 hover:text-gray-600">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showConfirm ? 'text' : 'password'} className={`${inputCls('password_confirmation')} pl-11 pr-12`} placeholder="Confirm your password" value={form.password_confirmation} onChange={set('password_confirmation')} required />
                  <button type="button" onClick={() => setShowConfirm(v => !v)} className="absolute inset-y-0 right-3.5 flex items-center text-gray-400 hover:text-gray-600">
              ...
              {/* Register button */}
              <button type="submit" disabled={loading} className="w-full bg-red-700 hover:bg-red-800 disabled:bg-red-300 text-white font-bold py-4 rounded-xl transition-colors text-base tracking-wide mt-2 flex items-center justify-center gap-2">
                {loading && (
            ...
            <p className="text-center text-sm text-gray-500 pt-3">
              ...
              <Link to="/donor/login" className="text-red-600 font-bold hover:text-red-700">Login</Link>
            </p>
